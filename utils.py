...
def resample_frames(frames, original_fps, target_fps):
    ...
    target_indices = np.linspace(0, frame_count - 1, target_frame_count).astype(int)
    resampled_frames = frames[target_indices]
...
def read_video(path: str):
    ...
    #flag_talknet = False
    flag_talknet = True
    #print('flag_talknet', flag_talknet)
    if not flag_talknet:
        video = video.permute(0, 3, 1, 2) / 255
    else:
        #video = torchvision.transforms.Grayscale()(video)
...
def _get_ResConv(PtConv):
    class _ResConvNd(Module):
...
ResConv3d = _get_ResConv(torch.nn.Conv3d)
